...
impl<T: Transport> ChainState for RpcChain<T> {
    fn block(&self) -> BlockInfo {
        ...
        let block = block_on(Tokio2::compat(self.connection.eth().block(block_id)))
    ...
    fn nonce(&self, address: &U256) -> usize {
        ...
        let nonce = block_on(Tokio2::compat(
            self.connection
                .eth()
    ...
    fn balance(&self, address: &U256) -> U256 {
        let address = H160::from_slice(&address.to_bytes_be()[12..]);
        let balance = block_on(Tokio2::compat(
            self.connection
                .eth()
    ...
    fn code(&self, address: &U256) -> Vec<u8> {
        let address = H160::from_slice(&address.to_bytes_be()[12..]);
        let code = block_on(Tokio2::compat(
            self.connection.eth().code(address, Some(self.block_number)),
        ))
    ...
    fn storage(&self, address: &U256, slot: &U256) -> U256 {
        ...
        let storage = block_on(Tokio2::compat(self.connection.eth().storage(
            address,
            idx,
